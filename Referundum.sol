...
contract Voting {
    ...
    struct Member {
        ...
        uint256 warnings; // 0 at registration, can be increased
        bool isBlacklisted; //false at registration and as long as warnings < 2, true if warnings >=2
        uint256 delayRegistration; // till when is the member registered following his payment
    }
    ...
    struct Proposal {
        ...
        uint256 counterBlankVotes; // ounter of votes `Blank`
        uint256 delay; // till when the proposal is active (proposal active for 1 week )
    ...
    modifier onlyAdmin() {
        require(
            members[msg.sender].isAdmin == true,
    ...
    modifier onlyActiveMembers() {
        require(
            ...
            "only members who have paid the registration till present can call this function"
        );
        _;
    }
    ...
    modifier onlyWhitelistedMembers() {
        require(
            members[msg.sender].isBlacklisted == false,
            "only members not blacklisted can call this function"
        );
        _;
    }
        ...
        onlyWhitelistedMembers
    {
        ...
        require(
            proposals[_id].delay > block.timestamp,
            "proposal not active any more"
        );
        require(
    ...
    {
        require(
            members[_addr].delayRegistration >= block.timestamp,
            " member to be set admin is behind with registration payment"
        );
        members[_addr].isAdmin = true;
    }
    ...
    function unsetAdmin(address _addr)
        public
        onlyAdmin
        onlyActiveMembers
        onlyWhitelistedMembers
